        ...
        <div class="col-md-6 col-lg-4 col-xl-3">
            <div class="product-card card h-100 border-0">
                <div class="product-img-container position-relative">
                    <img class="product-img main-img" src="{{ asset($product->product_image ?? 'images/no-image.jpg') }}"
                        alt="{{ $product->name }}">
                        ...
                        @if (!empty($product->colorData))
                            <div class="d-flex flex-wrap gap-2 mb-2">
                                @foreach ($product->colorData as $color)
                                    <span class="color-option" style="background-color: {{ $color['hex'] }};"
                                        data-image="{{ asset($color['image']) }}" data-name="{{ $color['variant_name'] }}"
                                        data-price="{{ number_format($color['price']) }} VNĐ" title="{{ ucfirst($color['name']) }}"
